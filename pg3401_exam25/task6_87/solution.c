...
int main(int iArgC, char **arrpszArgV){
   ...
   iReceived = recv(sockClient, aun_by8Encrypted, iContentLength, 0);
   if(iReceived < 0){
      printf("Failed to receive response from server - errcode %d\n", errno);
      close(sockClient); sockClient = -1;
      return -2; 
   ...
   for(i = 0; i < iContentLength / 8; i++){
      printf("0x%016llX ", aun_by8Encrypted[i].by8Base);
   }
   printf("\nENC END=\n");
   ...
   /* Getting file size */
   int iFileContent = iContentLength;
   ...
   for(i = 0; i <= 255; i++){
      ...
      if(iCheckAll != 1){
         ...
         if(l == iSize){
            ...
            printf("\n\n---RAN DECRYPT -> KEY=%016llx%016llx \n", by16Key.by8[0], by16Key.by8[1]);
            printf("\n%s\n\n", szDeciphered);
            int a;
            printf("DECYPHERED BYTES\n");
            for(a = 0; a < iSize; a++){
               printf("%016llX ", aun_by8Buffer[a].by8Base);
            }
